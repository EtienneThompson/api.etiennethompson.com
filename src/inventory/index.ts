...
export const getFolder = async (req: Request, res: Response) => {
  ...
  const getFolderQuery = `SELECT folderid, name, picture, description, created, modified FROM folders WHERE folderid='${params.folderid}' AND owner='${userid}'`;
...
export const getItem = async (req: Request, res: Response) => {
  ...
  // The client id was verified in middleware, so this should always return a value.
  let userid = rows[0].userid;
  const getItemQuery = `SELECT itemid, name, picture, description, created, updated FROM items WHERE itemid='${params.itemid}' AND owner='${userid}'`;

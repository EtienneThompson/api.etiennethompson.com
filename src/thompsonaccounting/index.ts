...
const getSingleClientDetails = async (
  ...
  let selectConditions: string[] = [];
  let joinConditions: string[] = [];
  // Get the schema for each table related to clients.

  for (let i = 0; i < tableNames.length; i++) {
    selectConditions.push(`${tableNames[i].column_name}.*`);
    joinConditions.push(
      `JOIN ${tableNames[i].column_name} ON clients.${tableNames[i].column_name} = ${tableNames[i].column_name}.${tableNames[i].column_name}_id`
    );
  }
